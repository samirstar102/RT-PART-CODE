...
@interface RTAutocompletingSearchViewController ()
...
@implementation RTAutocompletingSearchViewController {
...
+ (RTAutocompletingSearchViewController*) autocompletingSearchViewController {
  UIStoryboard* storyboard = [UIStoryboard storyboardWithName:@"RTAutocompletingSearchStoryboard" bundle:nil];
  return (RTAutocompletingSearchViewController*)[storyboard instantiateViewControllerWithIdentifier:@"SearchViewController"];
}
...
- (void) setDelegate:(NSObject<RTAutocompletingSearchViewControllerDelegate>*)delegate {
  _delegate = delegate;
...
- (void)setDataSource:(NSObject<RTAutoCompletingSearchViewControllerDataSource> *)dataSource {
    _dataSource = dataSource;
    
    self.results = [dataSource searchControllerDataSourceForSearch];
}
...
- (void) executeSearchForQuery:(NSString*)query searchID:(NSUInteger)searchID {
  ...
  [self setLoading:YES];
    ...
    for( int i = 0; i < [self.dataSource searchControllerDataSourceForSearch].count; i++ ) {
        NSString *item = [self.dataSource searchControllerDataSourceForSearch][i];
        if( [item.capitalizedString containsString:query.capitalizedString] ) {
            [results addObject:item];
        }
    }
    
    NSAssert(!searchResultsReturned, @"RTAutocompletingSearchController: delegate called results handler more than once for the same search execution.");
    ...
    if (searchID >= currentlyDisplaySearchID) {
      ...
      NSLog(@"RTAutocompletingSearchController: received out-of-order search results; ignoring. (currently displayed: %lu, searchID: %lu", (unsigned long)currentlyDisplaySearchID, (unsigned long)searchID);
    }
    [self decrementQueueCounter];
}
...
- (UITableViewCell*) tableView:(UITableView*)tableView cellForRowAtIndexPath:(NSIndexPath*)indexPath {
  ...
  if (row < self.results.count) {
    if (delegateManagesTableViewCells) {
      ...
      NSString* result = (NSString*)[self.results objectAtIndex:row];
      RTAutocompletingSearchGenericResultCell* cell = (RTAutocompletingSearchGenericResultCell*)[self.resultsTableView dequeueReusableCellWithIdentifier:@"ResultCell"];
      cell.resultLabel.text = result;
        
        UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(onSelectRow:)];
        [cell setTag:indexPath.row];
        [cell addGestureRecognizer:tap];
      return cell;
    }
  } else {
    NSLog(@"RTAutocompletingSearch: results table view attempted to load row %lu but only %lu result(s) exist.", (unsigned long)row, (unsigned long)self.results.count);
    return Nil;
  }
}

- (void)onSelectRow:(UITapGestureRecognizer *)tap {
    int indexRow = (int)tap.view.tag;
    [self.resultsTableView selectRowAtIndexPath:[NSIndexPath indexPathForRow:indexRow inSection:0] animated:YES scrollPosition:UITableViewScrollPositionNone];
    [self tableView:self.resultsTableView didSelectRowAtIndexPath:[NSIndexPath indexPathForRow:indexRow inSection:0]];
}
